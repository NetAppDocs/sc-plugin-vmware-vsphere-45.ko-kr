...
[role="lead"]
가상 어플라이언스를 구축하기 전에 구축 요구 사항을 숙지해야 합니다. 배포 요구 사항은 다음 네 개의 표에 나열되어 있습니다.
...
| 주 대상 | VMware SnapCenter 표준을 통해 애플리케이션 기반 보호를 수행하여 VMware VM 및 데이터 저장소에 대한 보호를 수행하려면 SnapRestore: 복원 작업에 사용됨 FlexClone: 마운트 및 연결 작업에 사용됨 


| 보조 대상 | VMware SnapCenter Standard를 통해 애플리케이션 기반 보호를 수행하려면 페일오버 작업에 VMware VM 및 데이터 저장소에 대한 보호를 수행하는 데 사용됩니다. FlexClone: 마운트 및 연결 작업에 사용됩니다 
|===
...
| vCenter vSphere를 참조하십시오 | HTML5 클라이언트: 6.5U2/U3, 6.7x, 7.0, 7.0U1, 7.0U2 Flex 클라이언트는 지원되지 않습니다. 
...
| VMware vSphere MySQL 데이터베이스용 SnapCenter 플러그인 | MySQL 8.0.23 
...
|===
지원되는 버전에 대한 최신 정보는 를 참조하십시오 https://mysupport.netapp.com/matrix/imt.jsp?components=91324;&solution=1517&isHWU&src=IMT["NetApp 상호 운용성 매트릭스 툴"^].
...
| VMware vSphere 포트용 SnapCenter 플러그인 | 8144(HTTPS), 양방향 이 포트는 VMware vSphere 웹 클라이언트 및 SnapCenter Server와의 통신에 사용됩니다. 8080 양방향 이 포트는 가상 어플라이언스를 관리하는 데 사용됩니다. 참고: 포트 구성은 수정할 수 없습니다. 
